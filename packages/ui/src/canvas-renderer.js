...
export class CGMRenderer {
    ...
    viewOffsetMs = 0; // 0 = live-follow; positive = panned into history
    isDragging = false;
    dragStartX = 0;
    dragStartOffset = 0;
    pinchStartDist = 0;
    ...
    get isLive() { return this.viewOffsetMs === 0; }
    get zoomMinutes() { return this.viewWindowMinutes; }
    onViewChange(cb) { this.viewChangeCallbacks.push(cb); }
    pushTick(snap) {
        this.ring.push({
    ...
    get hasComparison() { return this.comparisonRing.size > 0; }
    pushEvents(evs) {
        for (const ev of evs)
            this.events.push(ev);
        this.dirty = true;
    ...
    // ── Coordinate helpers ────────────────────────────────────────────────────
    get plotW() { return this.cssW - this.PAD_LEFT - this.PAD_RIGHT; }
    get plotH() { return this.cssH - this.PAD_TOP - this.PAD_BOTTOM; }
    toDisplay(mgdl) {
    ...
    setupPanZoom() {
        ...
        canvas.addEventListener('touchstart', (e) => {
            ...
            if (touches.length === 1) {
                ...
                this.isDragging = false;
                this.pinchStartDist = Math.hypot(touches[0].clientX - touches[1].clientX, touches[0].clientY - touches[1].clientY);
        ...
        canvas.addEventListener('touchend', () => {
            ...
            const levels = [180, 360, 720, 1440];
            this.viewWindowMinutes = levels.reduce((best, lvl) => Math.abs(lvl - this.viewWindowMinutes) < Math.abs(best - this.viewWindowMinutes) ? lvl : best);
            this.dirty = true;
            this.notifyViewChange();
        });
    }
    ...
    render() {
        ...
        if (this.options.showBasal)
            this.drawBasalOverlay(winStartMin);
        if (this.options.showCOB)
            this.drawCOBOverlay(winStartMin);
        if (this.options.showIOB)
            this.drawIOBOverlay(winStartMin);
        if (this.options.showTrueGlucose)
            this.drawTrueLine(winStartMin);
        ...
        if (this.options.showEvents)
            this.drawEventMarkers(winStartMin);
    }
    // ── Draw layers ───────────────────────────────────────────────────────────
    ...
    drawGrid(winStartMin) {
        ...
        for (const mg of glucoseLines) {
            ...
            if (y < this.PAD_TOP || y > this.PAD_TOP + this.plotH)
                continue;
            ctx.beginPath();
        ...
        for (let simMin = firstMark; simMin <= winStartMin + this.viewWindowMinutes; simMin += stepMin) {
            ...
            if (offsetMin < 0 || offsetMin > this.viewWindowMinutes)
                continue;
    ...
    drawBasalOverlay(winStartMin) {
        ...
        for (let i = 0; i < pts.length; i++) {
            ...
            if (i + 1 < pts.length)
                ctx.lineTo(pts[i + 1].x, p.y); // hold until next tick
        }
    ...
    computeAnimatedSimMs(latest) {
        ...
        if (!latest || !this.isRunning || this.viewOffsetMs > 0)
            return latest?.simTimeMs ?? 0;
    ...
    drawAnimatedExtension(winStartMin, latest, animSimMs) {
        if (!latest || !this.isRunning || this.viewOffsetMs > 0)
            return;
        if (animSimMs <= latest.simTimeMs)
            return;
        const lastOffsetMin = latest.simTimeMs / 60_000 - winStartMin;
        if (lastOffsetMin > this.viewWindowMinutes)
            return;

...
function v3Activity(peak: number, duration: number, minutesAgo: number, units: number): number {
  ...
  if (act <= 0) return 0;
  if (minutesAgo < 15) return act * (minutesAgo / 15);
  return act;
}

function v3IOB(peak: number, duration: number, minutesAgo: number, units: number): number {
  ...
  if (minutesAgo < 15) iobFraction = 1 - (minutesAgo / 15) * (1 - iobFraction);
...
describe('getExpTreatmentIOB — v3 parity (analytical biexponential)', () => {
  ...
  it('steady-state pump IOB ≈ 1.28 U at 0.8 U/h', () => {
    ...
    let iob = 0;
    for (let k = 0; k < 60; k++) iob += v3IOB(PEAK, DIA_MIN, k * 5, units);
    const v4iob = (() => {
...
describe('calculateBolusIOB — respects per-bolus DIA, not hard-coded profile', () => {
  it('returns 0 at t=200 min when bolus DIA is 3 h (not the 5 h Fiasp profile)', () => {
    ...
    const bolus = {
      id: 'test', simTimeMs: 0, units: 1, analogue: 'Fiasp' as const, dia: 3,
    };
    expect(calculateBolusIOB([bolus], 200 * 60_000)).toBe(0);
  });

  it('uses the stamped DIA for a shorter-than-profile duration', () => {
    const bolus = { id: 'test', simTimeMs: 0, units: 1, analogue: 'Fiasp' as const, dia: 4 };
  ...
  it('two-layer DIA: longer patient.dia means insulin lingers past programmed DIA', () => {
    ...
    const longDia = { id: 'a', simTimeMs: 0, units: 1, analogue: 'Fiasp' as const, dia: 7 };
    const shortDia = { id: 'b', simTimeMs: 0, units: 1, analogue: 'Fiasp' as const, dia: 5 };
...
describe('runPID — v3 PID-IFB parity', () => {
  ...
  const flatState = (rate: number): PIDState => ({
  ...
  it('with enableSMB false, microbolusUnits is always 0 even on rapid rise', () => {
    ...
    const state: PIDState = { cgmHistory: [100, 110, 120, 130], prevRate: BASE_RATE, ticksSinceLastMB: 999 };
  ...
  it('with enableSMB true, microbolus fires on rapid rise (≥2 mg/dL/min)', () => {
    ...
    const state: PIDState = { cgmHistory: [120, 125, 128, 130], prevRate: BASE_RATE, ticksSinceLastMB: 999 };
...
describe('deltaBG steady state — pump at ~0.75 U/h', () => {
  ...
  it('net deltaBG ≈ 0 at ~0.75 U/h after reaching steady state', () => {
    ...
    const microBoluses: Array<{ simTimeMs: number; units: number; dia: number; peak: number }> = [];

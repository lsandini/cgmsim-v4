 ...
 */
export function calculateInsulinSuppressionFactor(insulinActivity: number, weight: number): number {
  if (insulinActivity <= 0) return 1;
 ...
 */
export function calculateEGP(
  patient: VirtualPatient,
  simTimeMs: number,
  isf: number,
  insulinActivity: number = 0,
  ...
  if (currentGlucose !== undefined && currentGlucose < CR_ONSET_BG) {
    ...
    if (integrity > 0) {
      const depth = Math.max(0, Math.min(1,
        (CR_ONSET_BG - currentGlucose) / (CR_ONSET_BG - CR_FULL_BG)
      ));

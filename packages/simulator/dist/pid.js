 ...
 */
export function runPID(cgm, iob, therapy, state, basalRateUPerHour) {
    ...
    const newIntegral = Math.max(MIN_INTEGRAL, Math.min(MAX_INTEGRAL, state.integral + error * TICK_MINUTES));
    // PID output (additive adjustment above scheduled basal)
    const pidAdjustment = KP * error +
        KI * newIntegral +
        KD * dCGM;
    ...
    const clampedRate = Math.max(MIN_RATE_U_PER_HOUR, Math.min(MAX_RATE_U_PER_HOUR, rawRate * iobSafetyFactor));
    // ── Fuzzy microbolus layer ──────────────────────────────────────────────
    let microbolusUnits = 0;
    if (cgm > MICROBOLUS_CGM_THRESHOLD &&
        dCGM > MICROBOLUS_TREND_THRESHOLD &&
        iob < 2.0) {
        // Microbolus proportional to error above threshold, capped
        const fuzzyAmount = Math.min(MICROBOLUS_MAX_UNITS, (cgm - MICROBOLUS_CGM_THRESHOLD) / therapy.programmedISF * 0.3);
